...
class ScadsAgent:
    ...
    def __init__(
        ...
        """
        self.api_key = api_key or os.environ.get("SCADS_API_KEY") or (open(os.path.expanduser("~/.scads_api_key")).read().strip() if os.path.exists(os.path.expanduser("~/.scads_api_key")) else None)
        if not self.api_key:
            raise ValueError(
